...
export function selectBestXpTarget(servers) {
  return [...servers]
    .filter((server) =>
      server.rooted && server.maxMoney > 0 && server.requiredLevel <= server.hackingLevel
    )
    .sort((left, right) =>
      left.weakenTime - right.weakenTime ||
...
export function chooseNextBitNode(resetInfo, order) {
  const milestones = (order ?? []).map((entry) => {
    if (entry && typeof entry === "object") {
      return { node: Number(entry.node), level: Math.max(1, Number(entry.level) || 1) };
    }
    return { node: Number(entry), level: 3 };
  }).filter(({ node }) => Number.isFinite(node));
  const route = [...new Set(milestones.map(({ node }) => node))];
  ...
  const milestone = milestones.find(({ node, level }) =>
    projectedSourceFileLevel(resetInfo, node) < level
  );
  if (milestone) return milestone.node;
  ...
  // BN12 has no finite maximum. Once every configured finite Source-File goal
  // is complete, it is the permanent open-end destination.
  return 12;
}
