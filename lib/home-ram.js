...
export function fullOperationRamPlan(
  ns,
  files,
  config,
  phaseWorkerGroups = [],
  ...
  const hackingWorkers = new Set(hackingWorkerFiles);
  let managementReserve = [...uniqueFiles]
    .filter((file) => !hackingWorkers.has(file))
    .map((file) => Number(ns.getScriptRam(file, "home")) || 0)
    .reduce((sum, ram) => sum + Math.max(0, ram), 0);
  for (const group of phaseWorkerGroups) {
    const maximum = Math.max(0, ...group.map((file) => Number(ns.getScriptRam(file, "home")) || 0));
    if (maximum > 0) {
      ramValues.push(maximum);
      managementReserve += maximum;
    }
  }
  return {
    target: calculateConcurrentRamTarget(
      ramValues,
...
export function fullOperationRamTarget(ns, files, config, phaseWorkerGroups = []) {
  return fullOperationRamPlan(ns, files, config, phaseWorkerGroups).target;
}

export function homeRamUtilization(maxRam, usedRam) {
  const maximum = Number(maxRam);
  if (!Number.isFinite(maximum) || maximum <= 0) return 0;
  return Math.max(0, Number(usedRam) || 0) / maximum;
}

export function homeRamUnderPressure(maxRam, usedRam, threshold = 0.80) {
  const limit = Math.min(1, Math.max(0, Number(threshold) || 0));
  return homeRamUtilization(maxRam, usedRam) > limit;
}

export function fullModeHomeReserve(maxRam, managementReserve, threshold = 0.80) {
  const limit = Math.min(1, Math.max(0, Number(threshold) || 0));
  const saturationReserve = Math.max(0, Number(maxRam) || 0) * (1 - limit);
  return saturationReserve + Math.max(0, Number(managementReserve) || 0);
}

export function availableHomeWorkerRam(maxRam, usedRam, reserve) {
  return Math.max(
    0,
    Number(maxRam) - Number(usedRam) - Math.max(0, Number(reserve) || 0),
  );
}
...
export function readHomeRamFocus(ns) {
  try {
    ...
    return {
      ...
      managementReserve: Math.max(0, Number(state.managementReserve) || 0),
    };
  } catch {
    return {
      active: false,
...
export function writeHomeRamFocus(
  ns,
  current,
  target,
  mediumRatio = 0.5,
  ...
  const state = {
    ...
    mediumAt,
    managementReserve: Math.max(0, Number(managementReserve) || 0),
