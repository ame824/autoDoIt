...
export function availableHomeWorkerRam(maxRam, usedRam, workerRam, managementReserve) {
  const actualFree = Math.max(0, Number(maxRam) - Number(usedRam));
  const workerCapacityLeft = Math.max(
    0,
    Number(maxRam) - Math.max(0, Number(managementReserve) || 0) -
      Math.max(0, Number(workerRam) || 0),
  );
  return Math.min(actualFree, workerCapacityLeft);
}
