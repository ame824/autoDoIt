...
export function createNodeRushState({
  ...
  if (Number(currentNode) === 15 && !hasRedPill) {
    const targetCharisma = Math.max(0, Number(labyrinthRequiredCharisma) || 0);
    return {
      ...base,
      stage: targetCharisma > 0 && Number(charismaLevel) < targetCharisma
        ? "labyrinth-charisma"
        : "labyrinth",
      targetCharisma,
    };
...
export function parseNodeRushState(raw, now = Date.now()) {
  ...
  return {
    ...
    targetCharisma: Math.max(0, Number(state.targetCharisma) || 0),
  };
}

export function isCriticalPlayerTrainingStage(state) {
  return new Set([
    "daedalus-hacking",
    "world-daemon-hacking",
    "labyrinth-charisma",
  ]).has(String(state?.stage ?? ""));
}
