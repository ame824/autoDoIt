...
export function sortTasksForMode(tasks, mode) {
  const priority = (task) => {
    ...
    return Number(task.fullPriority ?? task.priority);
  };
...
export function effectiveManagerInterval(
  intervalMs,
  speedMultiplier = 1,
  minimumIntervalMs = 500,
) {
  const base = Math.max(0, Number(intervalMs) || 0);
  const speed = Math.max(1, Number(speedMultiplier) || 1);
  const minimum = Math.max(1, Number(minimumIntervalMs) || 1);
  return Math.max(minimum, Math.round(base / speed));
}
